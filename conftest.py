...
@allure.step('Регистрация нового тестового курьера с последующим удалением')
@pytest.fixture()
def response_courier():
    # отправляем запрос на регистрацию тестового курьера и сохраняем ответ в переменную response
    response_courier = requests.post(data.URL_COURIER, data.TEST_COURIER)
    id_courier = requests.post(data.URL_COURIER_LOGIN, data.TEST_COURIER).json()['id']
    yield response_courier
